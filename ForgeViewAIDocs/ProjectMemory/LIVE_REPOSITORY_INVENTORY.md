...
| `ForgeViewAIVideos/` | Generated video outputs. | generated | Do not edit manually. Regenerate only in video/runtime task. | Never commit by default. | Large binary files and generated artifacts. |
...
### Backup `ForgeViewAIBackup/`

| Filename | Bot type | Version | Location status | Safe to edit | Requires verification |
| --- | --- | --- | --- | --- | --- |
| `BTC Bot - v88 STABILITY FIX.json` | Spot | v88 | backup-only unless confirmed live | No. | Confirm whether Spot v88 should be restored to live `Exports/` or remain backup-only. |
...
| Path | Risk |
| --- | --- |
| `ForgeViewAIContent\bridge_config.json` | Runtime bridge config; may contain local endpoint or operational settings. |
| `ForgeViewAIContent\openai_config.json` | Likely OpenAI API config; possible API key risk. |
| `ForgeViewAIContent\telegram_config.json` | Likely Telegram token/chat config risk. |
| `ForgeViewAIContent\telegram_config.json — копия.json` | Duplicate Telegram config; possible stale secret copy. |
| `ForgeViewAIContent\video_provider_config.json` | Video provider config; possible provider key or endpoint risk. |
| `ForgeViewAIContent\youtube_client_secret.json` | YouTube OAuth client secret risk. |
| `ForgeViewAIContent\youtube_token.json` | YouTube OAuth token risk. |
| `ForgeViewAIContent\youtube_upload_config.json` | Upload runtime config; possible channel or credential-related settings. |
| `ForgeViewAIContent\bridge_config.example.json` | Example config; verify no real values before commit. |
| `ForgeViewAIContent\openai_config.example.json` | Example config; verify no real values before commit. |
| `ForgeViewAIContent\youtube_upload_config.example.json` | Example config; verify no real values before commit. |
...
| `ForgeViewAIBackup\...` | Archive may contain stale secrets, old configs, virtualenv/cache, and old generated artifacts. Treat all backup internals as high-risk until reviewed. |
...
- Treat `Exports/` as mixed-risk; identify whether a file is source, trading export, runtime response, or script before editing.
- Treat all real files in `ForgeViewAIContent/` as secret-bearing unless proven otherwise.
...
- Add a follow-up task to classify which files in `Exports/` are safe source vs runtime/generated.
...
- `ForgeViewAIContent/*.json` except scrubbed examples after review
- `ForgeViewAIVideos/`
- `Logs/`
...
- `**/work/`
- `**/output/`
- raw backup contents from `ForgeViewAIBackup/`
